...
function Slider({ type, data }) {
  ...
  useEffect(()=>{
    if(movie || serie) {
      dispatch(getData(`${type == 'movie' ? `movie/${movie}` : `tv/${serie}/credits`}`))
      dispatch(getData(`${type == 'movie' ? `movie/${movie}` : `tv/${serie}/credits`}`))
    }
  },[movie,serie])
  return (
    <div className={styles.slider}>
      ...
      {  <InfoSlide data={detailMovie} casts={castsMovie}  type={type} setMovie={setMovie} />}
      {  <InfoSlide data={detailSerie} casts={castsSerie}  type={type} setSerie={setSerie} />}
    </div>
  )
}
